...
3. **Falls keine automatische Wiederherstellung:**
   - Die Session-Dateien befinden sich in: `gui/session_cache/`
   - Prüfen Sie, ob die Dateien existieren:
     ```bash
     ls -lh gui/session_cache/
     ```
...
manager = SessionStateManager(Path("gui/session_cache"))
