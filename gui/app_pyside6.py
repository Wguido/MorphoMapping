...
class MorphoMappingGUI(QMainWindow):
    ...
    def load_features_and_gates(self):
        ...
        try:
            ...
            # Exclude patterns
            exclude_patterns = ["intensity", "Intensity", "saturation", "Saturation", "Raw pixel", "Bkgd", "All", "Max Pixel", "Median Pixel", "Raw", "Time", "Object Number"]
    ...
    def export_cluster_heatmap(self):
        ...
        try:
                ...
                for feature in features:
                    ...
                    if total_weight > 0:
                        ...
                        if isinstance(mean_val, (list, tuple, np.ndarray)):
                            mean_val = float(np.mean(mean_val)) if len(mean_val) > 0 else 0.0
                        elif not isinstance(mean_val, (int, float, np.number)):
                            mean_val = 0.0
                        cluster_means[feature] = float(mean_val)
                    else:
                        cluster_means[feature] = 0.0  # Use 0 instead of NaN for consistency
                
                heatmap_data.append(cluster_means)
            
            # Create DataFrame: rows = features, columns = clusters
            heatmap_df = pd.DataFrame(heatmap_data)
            heatmap_df = heatmap_df.set_index("Cluster").T  # Transpose: features as rows, clusters as columns
            ...
            # Create heatmap
            fig, ax = plt.subplots(figsize=(max(8, len(clusters) * 0.8), max(6, len(features) * 0.3)))
            ...
            try:
                ...
                im = ax.imshow(heatmap_z.values, cmap="RdYlBu_r", aspect='auto', vmin=-3, vmax=3)
                ax.set_xticks(range(len(heatmap_z.columns)))
                ax.set_xticklabels(heatmap_z.columns, rotation=45, ha='right')
            ...
            ax.set_title("Cluster-Feature Heatmap (Row-wise Z-score)", fontsize=14, fontweight="bold")
